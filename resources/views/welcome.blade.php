...
<body>
        ...
        <div class="hero-content">
            <div class="hero-text">
                <div class="hero-badge floating">
                    <i class="fas fa-crown"></i> Kos Terbaik 2024
                </div>
                <h1>
    ...
    <div class="kamar-card" 
         data-type="{{ strtolower($item->nama_kamar) }}" 
         data-status="{{ $item->status }}" 
         data-id="{{ $item->id }}">
        
        <div class="kamar-badge {{ $item->status_class }}">
            {{ $item->status_label }}
        </div>
        
        <div class="kamar-img-container">
            <img src="{{ $item->foto ? asset('storage/' . $item->foto) : 'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80' }}" 
                 alt="Kamar {{ $item->nomor_kamar }}" 
                 class="kamar-img">
        </div>
        ...
        <div class="kamar-info">
            <div class="kamar-header">
                <div>
                    <h3 class="kamar-name">Kamar {{ $item->nomor_kamar }}</h3>
                    <span class="kamar-type">{{ $item->nama_kamar }}</span>
                </div>
                <div class="kamar-price">
                    <span class="price-from">Mulai dari</span>
                    <div class="price-amount">{{ $item->formatHarga($item->harga_3bulan/3) }}</div>
                    <span class="price-period">/bulan</span>
                </div>
            </div>
            
            <p class="kamar-description">{{ $item->deskripsi }}</p>
            
            <div class="pricing-tabs">
                <div class="price-option active" 
                     onclick="selectPriceOption(this, {{ $item->id }})"
                     data-harga="{{ $item->harga_3bulan }}">
                    <div class="price-duration">3 Bulan</div>
                    <div class="price-value">{{ $item->formatHarga($item->harga_3bulan) }}</div>
                </div>
                <div class="price-option" 
                     onclick="selectPriceOption(this, {{ $item->id }})"
                     data-harga="{{ $item->harga_6bulan }}">
                    <div class="price-duration">6 Bulan</div>
                    <div class="price-value">{{ $item->formatHarga($item->harga_6bulan) }}</div>
                    @if($item->harga_6bulan < ($item->harga_3bulan * 2))
                    <div class="price-savings">
                        Hemat Rp {{ number_format(($item->harga_3bulan * 2) - $item->harga_6bulan, 0, ',', '.') }}
                    </div>
                    @endif
                </div>
                <div class="price-option" 
                     onclick="selectPriceOption(this, {{ $item->id }})"
                     data-harga="{{ $item->harga_1tahun }}">
                    <div class="price-duration">1 Tahun</div>
                    <div class="price-value">{{ $item->formatHarga($item->harga_1tahun) }}</div>
                    @if($item->harga_1tahun < ($item->harga_3bulan * 4))
                    <div class="price-savings">
                        Hemat Rp {{ number_format(($item->harga_3bulan * 4) - $item->harga_1tahun, 0, ',', '.') }}
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="kamar-facilities">
                @foreach($item->fasilitas_array as $fasilitas)
                <span class="facility-tag">{{ trim($fasilitas) }}</span>
                @endforeach
            </div>
            
            <div class="kamar-actions">
                <button class="btn-action btn-detail" 
                        onclick="viewKamarDetail({{ $item->id }})"
                        wire:click="$emit('showDetail', {{ $item->id }})">
                    <i class="fas fa-info-circle"></i> Detail
                </button>
                
                @if($item->status === 'available')
                <button class="btn-action btn-wa" 
                        onclick="contactWhatsApp('Kamar {{ $item->nomor_kamar }} - {{ $item->nama_kamar }}')">
